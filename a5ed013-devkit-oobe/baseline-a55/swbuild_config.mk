...
# Strip suffix from machine name for customer-facing file names (agilex5e_013b -> agilex5e)
KAS_MACHINE_STRIP := $(shell echo "$(KAS_MACHINE)" | sed 's/_013b$$//')
...
$(YOCTO_SD_WIC): output_files/$(REVISION)_hps_debug.core.rbf
	...
	# Rename console-image files to customer-facing names immediately after build
	cd $(YOCTO_SD_IMAGE_DIR) && \
		for ext in rootfs.wic rootfs.tar.gz rootfs.manifest; do \
			if [ -f gsrd-console-image-$(KAS_MACHINE).$$ext ]; then \
				cp -f gsrd-console-image-$(KAS_MACHINE).$$ext gsrd-console-image-$(KAS_MACHINE_STRIP).$$ext; \
			fi; \
		done && \
		for ext in rootfs.tar.gz rootfs.manifest; do \
			if [ -f console-image-minimal-$(KAS_MACHINE).$$ext ]; then \
				cp -f console-image-minimal-$(KAS_MACHINE).$$ext console-image-minimal-$(KAS_MACHINE_STRIP).$$ext; \
			fi; \
		done
...
sd-postprocess: output_files/$(REVISION)_hps_debug.sof
	...
	cd $(YOCTO_SD_IMAGE_DIR) && \
		...
		quartus_pfg -c ghrd.sof ghrd.jic -o device=MT25QU128 -o flash_loader=A5ED013BM16AE4SCS -o hps_path=u-boot-spl-dtb.hex -o mode=ASX4 -o hps=1 && \
...
$(SW_YOCTO_LINUX_SD_TARGET)-install-sw : \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/sdimage.tar.gz \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/sdimage.tar.gz.md5sum \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/console-image-minimal-$(KAS_MACHINE_STRIP).rootfs.tar.gz \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/console-image-minimal-$(KAS_MACHINE_STRIP).rootfs.manifest \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/gsrd-console-image-$(KAS_MACHINE_STRIP).rootfs.tar.gz \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_sd/gsrd-console-image-$(KAS_MACHINE_STRIP).rootfs.manifest \
...
$(KAS_YOCTO_IMAGE_DIR)/u-boot-spl-dtb.hex: output_files/$(REVISION)_hps_debug.core.rbf
	...
	# Rename core-image-minimal files to customer-facing names immediately after build
	cd $(KAS_YOCTO_IMAGE_DIR) && \
		for ext in rootfs_nor.ubifs rootfs.tar.gz rootfs.manifest rootfs.jffs2; do \
			if [ -f core-image-minimal-$(KAS_MACHINE).$$ext ]; then \
				cp -f core-image-minimal-$(KAS_MACHINE).$$ext core-image-minimal-$(KAS_MACHINE_STRIP).$$ext; \
			fi; \
		done
...
$(SW_YOCTO_LINUX_QSPI_TARGET)-install-sw : \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_qspi/core-image-minimal-$(KAS_MACHINE_STRIP).rootfs_nor.ubifs \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_qspi/core-image-minimal-$(KAS_MACHINE_STRIP).rootfs.tar.gz \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_qspi/core-image-minimal-$(KAS_MACHINE_STRIP).rootfs.manifest \
	$(INSTALL_ROOT_BINARIES)/software/yocto_linux_qspi/core-image-minimal-$(KAS_MACHINE_STRIP).rootfs.jffs2 \
