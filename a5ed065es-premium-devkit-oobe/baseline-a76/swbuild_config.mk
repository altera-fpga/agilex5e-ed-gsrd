...
#Build the Yocto image (depends on baseline_a76 RBF)
$(YOCTO_SD_WIC): output_files/baseline_a76_hps_debug.core.rbf
...
sd-postprocess: output_files/baseline_a76_hps_debug.sof
	cp -f software/yocto_linux/scripts/* $(YOCTO_SD_IMAGE_DIR)
	cp -f output_files/baseline_a76_yocto_linux_sd.sof $(YOCTO_SD_IMAGE_DIR)
	cp -f output_files/baseline_a76_hps_debug.sof $(YOCTO_SD_IMAGE_DIR)

	# run uboot script and quartus_pfg to generate uboot only JIC
	cd $(YOCTO_SD_IMAGE_DIR) && \
		./uboot_bin.sh && \
		cp baseline_a76_yocto_linux_sd.sof ghrd.sof && \
...
$(SW_YOCTO_LINUX_SD_TARGET)-install-sw : \
	...
	$(INSTALL_ROOT_BINARIES)/baseline_a76_yocto_linux_sd.sof \
	sd-postprocess \
...
software/yocto_linux/build/tmp/deploy/images/agilex5e/u-boot-spl-dtb.hex: output_files/baseline_a76_hps_debug.core.rbf
...
qspi-postprocess: output_files/baseline_a76_hps_debug.sof
	cp -f software/yocto_linux/scripts/* $(YOCTO_QSPI_IMAGE_DIR)
	cp -f output_files/baseline_a76_yocto_linux_qspi.sof $(YOCTO_QSPI_IMAGE_DIR)
	cp -f output_files/baseline_a76_hps_debug.sof $(YOCTO_QSPI_IMAGE_DIR)
	...
	cd $(YOCTO_QSPI_IMAGE_DIR) && \
		...
		cp baseline_a76_yocto_linux_qspi.sof ghrd.sof && \
...
$(SW_YOCTO_LINUX_QSPI_TARGET)-install-sw : \
	...
	$(INSTALL_ROOT_BINARIES)/baseline_a76_yocto_linux_qspi.sof \
	qspi-postprocess \
