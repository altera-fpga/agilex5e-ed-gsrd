...
**CUSTOM LINUX DEVICE TREE**
- Refer to the official base device tree source https://github.com/altera-fpga/linux-socfpga.git - arch/arm64/boot/dts/intel/socfpga_agilex5_socdk.dts and create custom device tree with the different name. The dts file name must be different (e.g. tsn_cfg2.dts) for the custom device tree (dts).
...
FIT image - kernel.itb will use the custom device tree if CUSTOM_LINUX_DTS_FILE is defined in kas/machine.yml, else it will use Linux Kernel in-tree device tree defined using LINUX_DTS_FILE in kas/machine.yml.

```
E.g.
build/tmp/deploy/images/<Machine Id>/socfpga_agilex5_socdk.dtb
build/tmp/deploy/images/<Machine Id>/tsn_cfg2.dtb
```
