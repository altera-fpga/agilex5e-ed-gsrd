...
- **kas/image**  
The image directory contains yaml configurations for core-image-minimal and console-image-minimal targets.
...
## meta-custom layer
- **conf**  
Contains layer configuration and UBIFS settings.
- **recipes-apps**  
...
## scripts
Contains helper scripts and configuration files for generating QSPI JIC images and NAND UBI images from Yocto build outputs.
Refer to ```scripts/README.md``` for detailed instructions on creating bootable images for QSPI and NAND flash.
...
### Steps to create NAND bootable binaries
Follow steps mentioned in ```scripts/README.md``` file under the "NAND Boot Artifacts" section.
